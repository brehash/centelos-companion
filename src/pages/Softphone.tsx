...
export default function Softphone() {
  ...
  const statusColor =
    phone.phoneStatus === "registered" ? "bg-green-500" :
    ...
    <div className="flex flex-col h-screen bg-card overflow-hidden relative">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-border">
        ...
        <span className="text-xs font-medium text-card-foreground truncate">
          {phone.hasExtension ? `Ext ${phone.extensionNumber}` : "Softphone"}
        </span>
        {currentWorkspace && (
          <span className="text-[10px] text-muted-foreground truncate">— {currentWorkspace.name}</span>
        )}
        <div className={cn("h-2 w-2 rounded-full ml-auto shrink-0", statusColor)} />
        <span className="text-[10px] text-muted-foreground capitalize">{phone.phoneStatus}</span>
      </div>
            ...
            {isIdle && phone.callStatus !== "ringing-in" && (
              <div className="p-4 space-y-3">
                <div className="relative flex gap-1">
                  <Input value={dialNumber} onChange={(e) => setDialNumber(e.target.value)} placeholder="Number or @name…" className="text-sm h-9" onKeyDown={(e) => { if (e.key === "Enter" && !isAtSearch) handleDial(); }} />
                  <Button variant="ghost" size="icon" className="h-9 w-9 shrink-0" onClick={() => setDialNumber((p) => p.slice(0, -1))} disabled={!dialNumber}>
                ...
                <Button className="w-full bg-accent hover:bg-accent/90 text-accent-foreground" onClick={handleDial} disabled={!dialNumber.trim() || phone.phoneStatus !== "registered"}>
                ...
                {(inputDevices.length > 0 || outputDevices.length > 0) && (
                  <div className="pt-2 border-t border-border">
                    <button onClick={() => setShowAudioSettings((v) => !v)} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors w-full">
                      <Headphones className="h-3 w-3" /><span>Audio Devices</span>
                      {showAudioSettings ? <ChevronUp className="h-3 w-3 ml-auto" /> : <ChevronDown className="h-3 w-3 ml-auto" />}
                    </button>
                    {showAudioSettings && (
                      <div className="space-y-2 mt-2 animate-in slide-in-from-top-2 duration-200">
                        {inputDevices.length > 0 && (
                          <Select value={selectedInput} onValueChange={(val) => { setSelectedInput(val); phone.setInputDevice(val); }}>
                            <SelectTrigger className="h-7 text-xs"><SelectValue placeholder="Microphone" /></SelectTrigger>
                            <SelectContent>{inputDevices.map((d) => <SelectItem key={d.deviceId} value={d.deviceId} className="text-xs">{d.label || `Mic ${d.deviceId.slice(0, 6)}`}</SelectItem>)}</SelectContent>
                          </Select>
                        )}
                        {outputDevices.length > 0 && (
                          <Select value={selectedOutput} onValueChange={(val) => { setSelectedOutput(val); phone.setOutputDevice(val); }}>
                            ...
                            <SelectContent>{outputDevices.map((d) => <SelectItem key={d.deviceId} value={d.deviceId} className="text-xs">{d.label || `Speaker ${d.deviceId.slice(0, 6)}`}</SelectItem>)}</SelectContent>
                          </Select>
                        )}
                      </div>
                    )}
